...
object Server {
  ...
  /**
    ...
    */
  val identityParserSink: Sink[ByteString, Future[Identity]] = reframedFlow.map(Identity.parse).toMat(Sink.head)(Keep.right)
  ...
  /**
    ...
    */
  val reintroduceOrdering: Flow[Event, Event, NotUsed] = Flow[Event].statefulMapConcat[Event] { () =>

    var nextExpected = 1
    val heap = mutable.PriorityQueue[Event]()(Ordering.by[Event, Int](_.sequenceNr).reverse)

    event => {
      heap.enqueue(event)

      var outputBuffer = Vector.empty[Event]
      while (heap.nonEmpty && heap.head.sequenceNr == nextExpected) {
        outputBuffer = outputBuffer :+ heap.dequeue()
        nextExpected += 1
      }
      outputBuffer
    }
  ...
  val followersFlow: Flow[Event, (Event, Followers), NotUsed] = Flow[Event].statefulMapConcat { () =>
    var followers: Followers = Map.empty[Int, Set[Int]]
    event => {
      event match {
        case Follow(_, fromUserId, toUserId) =>
          val existingFollowers = followers.getOrElse(fromUserId, Set.empty)
          followers += (fromUserId -> (existingFollowers + toUserId))
...
//    * Status Update:   All current followers of the From User ID should be notified
  ...
  case (Follow(_, fromUserId, toUserId), followers) => userId == toUserId
  case (Unfollow(_, fromUserId, toUserId), followers) => false
  case (Event.Broadcast(_), _) => true
  case (Event.PrivateMsg(_, fromUserId, toUserId), followers) => userId == toUserId
  //case (Event.StatusUpdate(_, fromUserId), followers) => followers.get(fromUserId).fold(false)(_.contains(userId))
  case (Event.StatusUpdate(_, fromUserId), followers) => followers.get(userId).fold(false)(_.contains(fromUserId)) //TODO this doesn't seem right
  }
...
class Server()(implicit executionContext: ExecutionContext, materializer: Materializer)
  ...
  /**
    ...
    */
  val eventsFlow: Flow[ByteString, Nothing, NotUsed] = Flow.fromSinkAndSourceCoupled[ByteString, Nothing](inboundSink, Source.maybe)
  ...
  /**
    ...
    * Status Update:   All current followers of the From User ID should be notified
    */
  def outgoingFlow(userId: Int): Source[ByteString, NotUsed] = broadcastOut.filter(isNotified(userId)).map { case (event, followers) => event.render }
...
//    clientIdPromise.future.map(id => actorSystem.log.info("Connected follower: {}", id.userId))
    ...
    val incoming: Sink[ByteString, NotUsed] = identityParserSink.mapMaterializedValue {
      future => clientIdPromise.completeWith(future)
      NotUsed //TODO not sure
    }
