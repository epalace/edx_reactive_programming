...
object SimpleStreaming extends ExtraStreamOps {
   ...
   */
  def recoverSingleElement(ints: Source[Int, NotUsed]): Source[Int, NotUsed] = ints.recover { case _: IllegalStateException => -1}
   ...
   */
  def sumUntilBackpressureGoesAway: Flow[Int, Int, _] = Flow[Int].conflate(_ + _)//.logAllEvents("events")
   ...
   */
  def keepRepeatingLastObservedValue: Flow[Int, Int, _] = Flow[Int].extrapolate(x => Iterator.continually(x))//.logAllEvents("events")
