...
object Transactor {
    ...
    /**
      ...
      */
    def apply[T](value: T, sessionTimeout: FiniteDuration): Behavior[Command[T]] = SelectiveReceive(30, idle(value, sessionTimeout).asInstanceOf[Behavior[Command[T]]])
    ...
    private def idle[T](value: T, sessionTimeout: FiniteDuration): Behavior[PrivateCommand[T]] = Behaviors.receivePartial {
            case (ctx, Begin(replyTo: ActorRef[ActorRef[Session[T]]])) =>
                val child: ActorRef[Session[T]] = ctx.spawnAnonymous(sessionHandler(value, ctx.self, Set.empty))
                ctx.watch(child)
                //ctx.schedule(sessionTimeout, child, Rollback[T])
    ...
    private def inSession[T](rollbackValue: T, sessionTimeout: FiniteDuration, sessionRef: ActorRef[Session[T]]): Behavior[PrivateCommand[T]] = Behaviors.receivePartial {
        ...
        case (ctx, RolledBack(session))  =>
            if (session == sessionRef) {
                ctx.stop(session) //ESTO FUE CLAVE
                //session ! Rollback()
